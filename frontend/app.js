...
(() => {
  ...
  const helpModal = {
    open() {
      document.querySelector('[data-help-backdrop]').hidden = false;
      document.querySelector('[data-help-modal]').hidden = false;
      document.body.style.overflow = 'hidden';
    },
    close() {
      document.querySelector('[data-help-backdrop]').hidden = true;
      document.querySelector('[data-help-modal]').hidden = true;
      document.body.style.overflow = '';
    },
    bind() {
  ...
  const userModal = {
    ...
    _renderCocons() {
      ...
      list.innerHTML = this._cocons.map(c => {
        const members = (c.member_first_names || []).filter(Boolean);
        const membersLine = members.length
          ? `<p class="cocon-manage-item__members">${members.map(escapeHtml).join(' · ')}</p>`
          : '';
        return `
  ...
  function init() {
    ...
    document.querySelectorAll('[data-action="open-auth"]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (tab) authModal.switchTab(tab);
      });
    });
